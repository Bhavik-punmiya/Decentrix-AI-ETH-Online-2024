...
type UseChainlinkCodeAgentContract = {
    code: string;
...
export function useChainlinkCodeAgentContract(): UseChainlinkCodeAgentContract {
    ...
    const handleRunAgent = useCallback(async (prompt: string) => {
        ...
        const codeGenerationQuery = `
            You are an AI assistant specializing in Chainlink blockchain technology. Your task is to provide accurate, concise, and helpful responses to user queries about Chainlink, using the knowledge base provided to you. Please follow these guidelines:
            
            1. Analyze the user's question carefully: "${prompt}"
            2. Use the provided knowledge base to formulate your response.z
